...
const NavbarIndex = () => {
  ...
  const slidingAnimation = (
    <motion.div
      layoutId="underline"
      className="absolute -bottom-2 left-0 h-[2.5px] w-full bg-MainRed z-10"
  ...
  return (
    <>
      {
        isDesktop && (
          <nav className="h-[75px] flex items-center justify-between px-[75px] shadow-[0_4px_6px_-1px_rgba(0,0,0,0.1),0_2px_4px_-1px_rgba(0,0,0,0.06)]">
            <div className="text-MainRed">
              <NavLink to='/' className='h-[60px] max-sm:h-[50px] w-auto flex items-center'>
                <img
                  src={Logo}
            ...
            <ul className="flex justify-between gap-8 font-semibold text-sm max-md:gap-4 max-md:text-[13px]">
              ...
              >
                ...
                <div
                  className={`absolute top-full -left-4 mt-2 w-48 bg-white rounded-md shadow-lg z-10 overflow-hidden transition-all duration-300 ease-in-out ${showIntro ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2 pointer-events-none'
                    }`}
                >
                  <ul>
      ...
      {!isDesktop && (
        <nav className='px-10 w-full bg-white relative shadow-[0_4px_6px_-1px_rgba(0,0,0,0.1)]'>
          <div className='flex items-center w-full justify-between py-2 '>
            <Ham
              className='w-[30px] h-[30px] max-sm:w-[26px] max-sm:h-[26px] cursor-pointer'
